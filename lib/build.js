...
module.exports = function (obj, cb) {
    ...
    options.isCom ? doArr.push(toEs5) : doArr.push(distHtml);
    //test
    doArrDone(doArr);
    ...
    function distHtml(callback) {
        var files = glob.sync(['**/*.html', '!**/*.jst.html'], {cwd: srcBase});
        var i = 0;
        ...
        function compileA(item) {
            ...
            new compile({
                filePath: path.join(srcBase, item),
                isTms: false,
                name: config.name || '',
                group: config.group || '',
                ...
                transform: function (content, that) {
                    ...
                    var outputPath = options.outputPath || '//' + (options.host || 'dev.cdn.myweimai.com/assets') + '/' + (config.group || 'm') + '/' + config.name + '/' + that.getVersion();
